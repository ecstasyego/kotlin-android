...
### Example02: Parallelized Retrofit2 and Room on Service
#### File System
```
.Project
├── app
...
class RemoteService : Service() {
    ...
    private suspend fun csvDownload(fileName: String) {
        val code = fileName.split(".")[0]
        val call = apiService.downloadCsvFile(fileName)
    ...
    private fun sendFailureBroadcast(code: String) {
        sendBroadcast(
            Intent("com.example.myapplication.CSV_DOWNLOAD_COMPLETED").setPackage(packageName).apply {
